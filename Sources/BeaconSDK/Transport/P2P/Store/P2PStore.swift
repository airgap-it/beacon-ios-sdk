        ...
        class Associated: StoreProtocol {
            ...
            private func createState(completion: @escaping (Result<State, Swift.Error>) -> ()) {
                findRelayServer { relayServerResult in
                    ...
                    let state = P2P.Store.State(relayServer: relayServer, availableNodes: self.matrixNodes.count)
            ...
            private func updatePeerRelayServer(
                ...
                guard let senderIdentifier = try? Identifier(fromValue: sender) else {
                    completion(.success(()))
                    return
                }
                ...
                storageManager.findPeers(
                    where: {
                        do {
                            ...
                            let peerIdentifier = try self.communicationUtils.recipientIdentifier(
                    ...
                    self.storageManager.add(
                        ...
                        overwrite: true,
                        compareBy: { lhs, rhs in lhs.common.publicKey == rhs.common.publicKey },
