...
class StoreBox<S: StoreProtocol> {
    let associated: S
    ...
    func state(completion: @escaping (Result<S.State, Error>) -> ()) {
    ...
    func intent(action: S.Action, completion: @escaping (Result<(), Error>) -> () = { _ in /* no action */ }) {
